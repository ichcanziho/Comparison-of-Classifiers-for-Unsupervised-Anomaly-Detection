...
def runByScaler(root, scaler, models, start, counts, other_models, CPUS, save_name):
    results = Parallel(n_jobs=CPUS) \
        (delayed(getResults)
         (root, scaler, model, model_name, start, counts, other_models) for model_name, model in models.items())
    save_path = f'results/{scaler}/{save_name}_{scaler}_scaler_{start}_{start + counts}'
    saveResults(results, start, counts, name=save_path)
